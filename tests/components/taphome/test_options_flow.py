...
from homeassistant.components.taphome.const import CONF_IP
...
async def test_options_edit_button_multi_enum(hass: HomeAssistant, mock_hub) -> None:
    ...
    assert result["type"] is FlowResultType.CREATE_ENTRY
    assert entry.options["buttons"] == [{"id": 8, "actions": ["press", "long_press"]}]
...
async def test_options_edit_skips_unmatchable_registry_entries(
    ...
    registry.async_get_or_create(
        "light", "taphome", "custom-unique", config_entry=entry
    )
